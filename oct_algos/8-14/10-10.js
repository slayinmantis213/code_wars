...
function findObjectsFilter(searchObj, items) {
    const result = [];
    for(let item of items){// outer array for loop
        let push = true;
        for(let key in searchObj){//inner loop for objects
            if(item[key]!=searchObj[key]){//for each item in array compare every key to search object
                push = false;
            }
        }
        push ? result.push(item) : null; 
    } 
    return result;
}
